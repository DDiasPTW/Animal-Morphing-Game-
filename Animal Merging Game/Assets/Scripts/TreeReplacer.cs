    ...
    public void Convert()
{
    foreach (Terrain terrain_ in Terrain.activeTerrains)
    {
        ...
        foreach (TreeInstance tree in data.treeInstances)
        {
            Vector3 position = Vector3.Scale(tree.position, data.size); // Correctly scale position to terrain size

            // Adjust the position to be relative to the terrain's position
            position += terrain_.GetPosition();
